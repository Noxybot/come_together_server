...
DBMongo::DBMongo(const boost::property_tree::ptree& config)
    : m_client(mongocxx::uri(config.get<std::string>("mongo.connection_string", "mongodb://localhost:27017")))
{
    const auto db = m_client["come_together"];
    m_users_collection = db["users"];
    m_markers_collection = db["markers"];
    m_chat_collection = db["chats"];
                                              ...
                                              std::string& out_access_token) try
{
    ...
    out_uuid = email;
    const auto maybe_result = m_users_collection.find_one(document{} << "email" << email << finalize);
    if (maybe_result)
    {
     m_users_collection.update_one(document{} << "email" << email << finalize,
    ...
    else
    {
        const auto res = document{} << "email" << email
                ...
                << "name" << decoded.get_payload_claims()["name"].as_string()
                << finalize;
        m_users_collection.insert_one(res.view());
    }
...
std::array<std::string, 2> DBMongo::AddMarker(const CT::marker_info& info)
{
    const auto marker = document{} << "category" << info.cat()
            ...
            << finalize;
    const auto res = m_markers_collection.insert_one(marker.view());
    if (!res)
    {
    ...
    const auto chat = document{} << "marker_id" << marker_id
         ...
         << finalize;
    const auto chat_res = m_chat_collection.insert_one(chat.view());
    if (!chat_res)
    {
